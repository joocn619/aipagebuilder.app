        ...
        {(c.testimonials || []).map((t, i) => (
          <div key={i} className="flex flex-col rounded-2xl border p-6 shadow-sm"
            ...
            {(t.rating || 0) > 0 && (
              <div className="mb-4 flex gap-0.5">
                {Array.from({ length: 5 }).map((_, s) => (
                  <svg key={s} className="h-4 w-4" fill={s < (t.rating || 0) ? "#f59e0b" : "rgba(255,255,255,0.15)"} viewBox="0 0 20 20">
            ...
            <p className="flex-1 text-sm leading-relaxed"
              style={{ color: textCol ? textCol + "bb" : "rgba(203,213,225,0.9)" }}>{t.content}</p>
            {/* Author */}
            <div className="mt-5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs" style={{ color: textCol ? textCol + "80" : "rgba(148,163,184,0.8)" }}>
